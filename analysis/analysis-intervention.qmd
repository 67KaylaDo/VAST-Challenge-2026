...
- SwiftWren was the **third** campaign to post, following HiddenOrca (May 10) and MellowOtter (May 11) - a repeatable failure, not a glitch.
...
**What it shows.** Each campaign follows the same shape - an instruction file, a propagated task, and a final file-sourced post. The May 17 SwiftWren post was the third in nine days, following HiddenOrca on May 10 and MellowOtter on May 11; neither earlier campaign triggered a logged incident-response action. SwiftWren is the most persistent of the three, spanning the longest window and touching the widest set of agents - it is the largest campaign on every axis, while MellowOtter was a smaller run between the two larger ones. What recurs is the mechanism, not a steady escalation: the same relay-then-post shape appears in all three, which reads as one worm reused rather than three unrelated events.
...
**What it shows.** The three campaigns differ in scale - SwiftWren is the largest on every measure (relay count, duration, and agent reach), with MellowOtter the smallest and HiddenOrca in between - but the underlying mechanism is constant across all three. Each uses the same `_further_instructions.md` relay pattern ahead of a `content_source` post. This is the key structural fact for the intervention: because the mechanism never changes, a single rule targeting that mechanism covers every campaign at once, including any future campaign that reuses the pattern.
...
::: {.g-col-4}
#### Layer 1 — Relay block · *primary*
- Covers all **235** malicious queue relays
- **0** false positives among legitimate queue tasks
- Stops propagation before any payload reaches a poster
:::

::: {.g-col-4}
#### Layer 2 — Post gate · *secondary*
- Catches all **3** anomalous file-sourced posts
- No legitimate post uses `content_source`
- Reactive safety stop at the point of publication
:::

::: {.g-col-4}
#### Layer 3 — Cross-dept guard · *tertiary*
- Disrupts relays crossing org boundaries
- Targets the worm's structural spread
- Governance control — needs tuning
:::
